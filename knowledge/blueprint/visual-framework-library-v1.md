...
Purpose: Show the colliding macro forces reshaping the category as a single system, not a list.
Inputs: 3–5 forces (category, consumer, technology, competitive, cultural shift), each interpreted.
...
Inputs: strategic barriers, communication issues, competitive weaknesses, internal challenges.
...
Purpose: Manufacture urgency by making inaction concrete and four-dimensional.
Inputs: commercial, audience, competitive, brand risk.
Outputs: cost_of_inaction, urgency_case.
...
Inputs: routes scored on market size, speed, commercial value, strategic fit, defensibility.
...
Purpose: Commit to one bet and justify why alternatives are weaker.
Inputs: opportunity landscape, prioritisation criteria.
Outputs: primary_strategic_bet, success_definition.
...
Inputs: behaviours, motivations, beliefs, aspirations, media habits, category relationships.
...
Purpose: Reveal an overlooked, under-served, commercially large audience.
Inputs: underserved segments, barriers removed, future demand pool.
Outputs: audience_gap, secondary_audience_definition.
...
Inputs: two axes, competitor positions, client position.
...
Purpose: State the competitive position simply, memorably, defendably.
Inputs: we are, we are not, because.
...
Inputs: consumer desire, category delivery.
...
Purpose: Turn tension into a story the company can operate from.
Inputs: tension, shift, resolution, meaning.
Outputs: core_narrative, narrative_line.
...
Inputs: master narrative, territories, proof points, execution examples.
...
Purpose: Define recognisable assets that compound brand memory.
Inputs: visual, verbal, sonic, experiential, creator assets.
Outputs: asset_ecosystem, distinctive_assets_to_build.
...
Inputs: channels mapped to discovery, education, trust, conversion, retention.
...
Purpose: Explain how the brand becomes memorable, grounded in attention/memory science (Field, Binet, Sharp, Nelson-Field, System1).
Inputs: salience, distinctiveness, emotional memory, contextual effectiveness.
Outputs: attention_framework, attention_advantage.
...
Inputs: narrative, campaign territories, concepts, assets, distribution, optimisation.
...
Purpose: Sequence near-term action into phases prioritised by impact and learning.
Inputs: foundation, launch, scale actions.
Outputs: 90_day_roadmap.
...
Inputs: leading, lagging, commercial indicators, north star.
...
Purpose: Leave one unforgettable, repeatable truth (≤25 words).
Inputs: strategic principle.
Outputs: final_principle.
...
Why it's hard → Growth Friction Framework
Why act now → Risk Matrix
Where to play (survey) → Opportunity Landscape
Where to play (commit) → Opportunity Prioritisation Model
Who → Audience Ecosystem + Audience Gap Model
How to be different (geometry) → Positioning Map
How to be different (statement) → We Are / We Are Not
The story foundation → Tension Framework → Narrative Framework
Make it briefable → Messaging Pyramid + Asset Ecosystem
Make it grow → Channel Role Map + Attention Framework + Campaign Factory
Make it happen → Roadmap + Measurement Pyramid
Make it stick → Single Statement
