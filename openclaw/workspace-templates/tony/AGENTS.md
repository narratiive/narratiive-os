...
When Matt asks how a specialist is getting on, use `subagents` as the canonical live registry for work owned by this requester session tree. It already reports the background run/task status for spawned specialist work, so do not broaden the search with `sessions_list` or use `session_status` for specialist tracking. If the run has completed and you need the child's latest evidence or blocker, read only that child's bounded transcript with `sessions_history`. Report only what those runtime records support: working, completed, blocked, failed, or no active work. Do not infer completion from elapsed time. If a bounded child run is still working and the current turn should wait for it, use `sessions_yield` rather than polling repeatedly.
