...
def build_app() -> LeadAwareTonyApplication:
    ...
    lead_path = Path(os.getenv("TONY_INBOUND_LEADS_PATH", str(REPOSITORY_ROOT / ".runtime" / "inbound-leads.json"))).resolve(); lead_store = FileInboundLeadStore(lead_path); authoritative_lead_loader = build_authoritative_lead_loader(lead_store)
    ...
    capability_service = TonyCapabilityCommandService(executive_service); commercial_watch_service = TonyCommercialWatchCommandService(capability_service, store_path=Path(os.getenv("TONY_COMMERCIAL_COMMITMENTS_PATH", str(REPOSITORY_ROOT / ".runtime" / "commercial-commitments.json")))); agency_focus_service = TonyPersistentAgencyFocusCommandService(commercial_watch_service, store_path=Path(os.getenv("TONY_AGENCY_FOCUS_CONTEXT_PATH", str(REPOSITORY_ROOT / ".runtime" / "agency-focus-context.json")))); outcome_service = TonyOutcomeAccountabilityCommandService(agency_focus_service, store_path=Path(os.getenv("TONY_EXECUTIVE_OUTCOMES_PATH", str(REPOSITORY_ROOT / ".runtime" / "executive-outcomes.json")))); outcome_evidence_service = TonyOutcomeEvidenceCommandService(outcome_service)
    executive_learning_path = Path(os.getenv("TONY_EXECUTIVE_LEARNING_PATH", str(REPOSITORY_ROOT / ".runtime" / "executive-learning.json"))); learning_service = TonyExecutiveLearningCommandService(outcome_evidence_service, store_path=executive_learning_path); adaptive_service = TonyAdaptiveResponseCommandService(learning_service, learning_store_path=executive_learning_path); memory_service = TonyMemoryCommandService(adaptive_service, ExecutiveMemoryStore(Path(os.getenv("TONY_EXECUTIVE_MEMORY_PATH", str(REPOSITORY_ROOT / ".runtime" / "executive-memory.jsonl")))), agency_id=workspace_id)
    live_dispatchers = build_http_dispatchers(); dispatch_service = TonyCommercialAutonomousJudgementCommandService(memory_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_AUTONOMOUS_RESULT_CONTEXT_PATH", str(REPOSITORY_ROOT / ".runtime" / "autonomous-result-context.json"))))
    ...
    post_send_sync_service = TonyPostSendNotionSyncCommandService(dispatch_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_POST_SEND_NOTION_SYNC_PATH", str(REPOSITORY_ROOT / ".runtime" / "post-send-notion-sync.json")))); followup_service = TonyCommercialFollowupCommandService(post_send_sync_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_COMMERCIAL_FOLLOWUP_PATH", str(REPOSITORY_ROOT / ".runtime" / "commercial-followup.json")))); meeting_reply_service = TonyMeetingReplyPreparationCommandService(followup_service, dispatchers=live_dispatchers, verified_result_sink=accept_verified_commercial_result); meeting_booking_service = TonyConfirmedMeetingBookingCommandService(meeting_reply_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_MEETING_BOOKING_PATH", str(REPOSITORY_ROOT / ".runtime" / "meeting-booking.json")))); booking_sync_service = TonyPostBookingNotionSyncCommandService(meeting_booking_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_POST_BOOKING_NOTION_SYNC_PATH", str(REPOSITORY_ROOT / ".runtime" / "post-booking-notion-sync.json")))); discovery_outcome_service = TonyDiscoveryOutcomeTrackingCommandService(booking_sync_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_DISCOVERY_OUTCOME_TRACKING_PATH", str(REPOSITORY_ROOT / ".runtime" / "discovery-outcome-tracking.json")))); post_discovery_service = TonyPostDiscoveryCommercialCommandService(discovery_outcome_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_POST_DISCOVERY_COMMERCIAL_PATH", str(REPOSITORY_ROOT / ".runtime" / "post-discovery-commercial.json")))); proposal_execution_service = TonyPostDiscoveryProposalExecutionCommandService(post_discovery_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_POST_DISCOVERY_PROPOSAL_EXECUTION_PATH", str(REPOSITORY_ROOT / ".runtime" / "post-discovery-proposal-execution.json")))); proposal_outcome_service = TonyProposalOutcomeTrackingCommandService(proposal_execution_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_PROPOSAL_OUTCOME_TRACKING_PATH", str(REPOSITORY_ROOT / ".runtime" / "proposal-outcome-tracking.json")))); commercial_close_service = TonyCommercialCloseCommandService(proposal_outcome_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_COMMERCIAL_CLOSE_PATH", str(REPOSITORY_ROOT / ".runtime" / "commercial-close.json")))); delivery_bootstrap_service = TonyDeliveryBootstrapCommandService(commercial_close_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_DELIVERY_BOOTSTRAP_PATH", str(REPOSITORY_ROOT / ".runtime" / "delivery-bootstrap.json")))); drive_workspace_service = TonyDriveDeliveryWorkspaceCommandService(delivery_bootstrap_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_DRIVE_DELIVERY_WORKSPACE_PATH", str(REPOSITORY_ROOT / ".runtime" / "drive-delivery-workspace.json")))); delivery_commissioning_service = TonyDeliveryCommissioningCommandService(drive_workspace_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_DELIVERY_COMMISSIONING_PATH", str(REPOSITORY_ROOT / ".runtime" / "delivery-commissioning.json")))); delivery_blueprint_review_service = TonyDeliveryBlueprintReviewCommandService(delivery_commissioning_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_DELIVERY_BLUEPRINT_REVIEW_PATH", str(REPOSITORY_ROOT / ".runtime" / "delivery-blueprint-review.json")))); blueprint_client_delivery_service = TonyBlueprintClientDeliveryCommandService(delivery_blueprint_review_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_BLUEPRINT_CLIENT_DELIVERY_PATH", str(REPOSITORY_ROOT / ".runtime" / "blueprint-client-delivery.json")))); blueprint_delivery_notion_sync_service = TonyBlueprintDeliveryNotionSyncCommandService(blueprint_client_delivery_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_BLUEPRINT_DELIVERY_NOTION_SYNC_PATH", str(REPOSITORY_ROOT / ".runtime" / "blueprint-delivery-notion-sync.json")))); blueprint_client_feedback_service = TonyBlueprintClientFeedbackCommandService(blueprint_delivery_notion_sync_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_BLUEPRINT_CLIENT_FEEDBACK_PATH", str(REPOSITORY_ROOT / ".runtime" / "blueprint-client-feedback.json")))); blueprint_revision_service = TonyBlueprintRevisionCycleCommandService(blueprint_client_feedback_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_BLUEPRINT_REVISION_CYCLE_PATH", str(REPOSITORY_ROOT / ".runtime" / "blueprint-revision-cycle.json")))); blueprint_revision_persistence_service = TonyBlueprintRevisionPersistenceCommandService(blueprint_revision_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_BLUEPRINT_REVISION_PERSISTENCE_PATH", str(REPOSITORY_ROOT / ".runtime" / "blueprint-revision-persistence.json")))); execution_status_service = TonyVerifiedExecutionStatusCommandService(blueprint_revision_persistence_service); app.command_service = TonyTerminologyCommandService(execution_status_service); return LeadAwareTonyApplication(app, lead_store)
