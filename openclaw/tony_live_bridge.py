...
def build_app() -> LeadAwareTonyApplication:
    ...
    executive_outcomes_path = Path(
        os.getenv(
            "TONY_EXECUTIVE_OUTCOMES_PATH",
            str(REPOSITORY_ROOT / ".runtime" / "executive-outcomes.json"),
        )
    )
    outcome_service = TonyOutcomeAccountabilityCommandService(
    ...
    adaptive_service = TonyAdaptiveResponseCommandService(
        learning_service,
        learning_store_path=executive_learning_path,
    )
    memory_path = Path(
        os.getenv(
