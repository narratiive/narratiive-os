...
def build_app() -> LeadAwareTonyApplication:
    ...
    blueprint_client_feedback_service = TonyBlueprintClientFeedbackCommandService(blueprint_delivery_notion_sync_service, dispatchers=live_dispatchers, store_path=Path(os.getenv("TONY_BLUEPRINT_CLIENT_FEEDBACK_PATH", str(REPOSITORY_ROOT / ".runtime" / "blueprint-client-feedback.json"))))
    execution_status_service = TonyVerifiedExecutionStatusCommandService(blueprint_client_feedback_service)
