...
class TonyLiveBridgeTests(unittest.TestCase):
    ...
    def test_build_app_composes_terminology_dispatch_memory_focus_commercial_capability_and_executive_commands(self):
        ...
        persistence=execution_status.command_service; self.assertIsInstance(persistence,TonyBlueprintRevisionPersistenceCommandService); self.assertEqual(persistence.dispatchers,{})
        revision=persistence.command_service; self.assertIsInstance(revision,TonyBlueprintRevisionCycleCommandService); self.assertEqual(revision.dispatchers,{})
        feedback=revision.command_service; self.assertIsInstance(feedback,TonyBlueprintClientFeedbackCommandService); self.assertEqual(feedback.dispatchers,{})
        notion_sync=feedback.command_service; self.assertIsInstance(notion_sync,TonyBlueprintDeliveryNotionSyncCommandService); self.assertEqual(notion_sync.dispatchers,{})
