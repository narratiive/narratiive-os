...
class TonyLiveDeliveryCommissioningCompositionTests(unittest.TestCase):
    def _env(self, tmp):
        return {
            ...
            "TONY_BLUEPRINT_REVISION_PERSISTENCE_PATH": str(Path(tmp) / "blueprint-revision-persistence.json"),
        }
    ...
    def test_live_bridge_composes_delivery_blueprint_review_after_commissioning(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch.object(tony_live_bridge, "build_base_app", return_value=base_app), mock.patch.dict("os.environ", self._env(tmp), clear=True):
        ...
        persistence = execution_status.command_service
        self.assertIsInstance(persistence, TonyBlueprintRevisionPersistenceCommandService)
        self.assertEqual(persistence.dispatchers, {})
        revision = persistence.command_service
    ...
    def test_live_bridge_passes_configured_claude_dispatcher_to_delivery_layers(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            env = self._env(tmp)
            with mock.patch.object(tony_live_bridge, "build_base_app", return_value=base_app), mock.patch.object(tony_live_bridge, "build_http_dispatchers", return_value={"Claude": fake_dispatcher}), mock.patch.dict("os.environ", env, clear=True):
                app = tony_live_bridge.build_app()
        persistence = app.command_service.command_service.command_service
        self.assertIsInstance(persistence, TonyBlueprintRevisionPersistenceCommandService)
        self.assertIs(persistence.dispatchers["Claude"], fake_dispatcher)
        revision = persistence.command_service
        self.assertIsInstance(revision, TonyBlueprintRevisionCycleCommandService)
