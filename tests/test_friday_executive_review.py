...
class FridayExecutiveReviewServiceTests(unittest.TestCase):
    ...
    def test_uses_deterministic_seven_day_window_and_workspace_scope(self):
        ...
        self.assertEqual(review.period_start, "2026-07-17T18:00:00+01:00")
        self.assertEqual(review.period_end, "2026-07-24T18:00:00+01:00")

    def test_duplicate_records_are_excluded(self):
        duplicate = self.record(
            "same",
            "2026-07-20T09:00:00+01:00",
            ReviewRecordType.WIN,
            "One win",
        )
        review = self.service.build(
            (duplicate, duplicate),
    ...
    def test_patterns_require_three_records_for_established_confidence(self):
        ...
        self.assertEqual(review.patterns[0].count, 3)

    def test_weak_patterns_are_labelled_early_hypothesis(self):
        review = self.service.build(
            (
                self.record(
                    "theme-1",
                    "2026-07-20T09:00:00+01:00",
                    ReviewRecordType.THEME,
                    "Observed once",
                    theme="slow review",
                ),
            ),
        ...
        self.assertEqual(review.patterns[0].confidence, "early_hypothesis")

    def test_blocker_drives_next_week_recommendation(self):
        review = self.service.build(
            (
                self.record(
                    "blocked",
                    "2026-07-20T09:00:00+01:00",
                    ReviewRecordType.BLOCKED,
    ...
    def test_compact_render_is_bounded(self):
        review = self.service.build(
            tuple(
                self.record(
                    f"item-{index}",
                    f"2026-07-{18 + index}T09:00:00+01:00",
                    ReviewRecordType.COMPLETED,
                    "A" * 500,
                )
