...
class TonyCommercialAutonomousJudgementCommandService(TonyPersistentAutonomousResultCommandService):
    ...
    _AUTO_REPLY_MARKERS = ("automatic reply", "auto reply", "autoreply", "out of office", "away from the office", "currently away")
    _DECLINE_MARKERS = ("not interested", "no thanks", "no thank you", "not for us", "not a fit", "please unsubscribe", "remove me", "we'll pass", "we will pass")
    _MEETING_MARKERS = ("let's talk", "lets talk", "book a call", "set up a call", "schedule a call", "happy to chat", "worth a conversation", "availability next week", "when are you free", "when can you talk")
    _INFORMATION_MARKERS = ("tell me more", "send over", "more information", "more info", "can you explain", "what would this involve", "what does this involve")
    ...
    def execute(self, command: str, objects: Iterable[dict[str, Any]]) -> CommandResponse:
        ...
        if disposition == "growth_blueprint_ready":
            suffix = f" I reviewed the returned Growth Blueprint against the evidence and quality gate. It is ready for your approval. I recommend: {recommendation} Nothing has been sent externally."
        elif disposition == "growth_blueprint_revision_required":
            ...
            suffix = f" The Calendar check returned verified availability for this commercial lead. I recommend: {recommendation}"
        elif disposition == "meeting_draft_ready":
            suffix = f" I reviewed the returned meeting draft against the verified Calendar availability and lead context. It is ready for approval. I recommend: {recommendation} Nothing has been sent externally."
        elif disposition == "meeting_draft_revision_required":
            failed = ", ".join(str(item) for item in judgement.get("failed_checks", ()))
            suffix = f" I reviewed the returned meeting draft and would not send it yet. It needs revision on: {failed or 'the verified meeting-draft requirements'}. Nothing has been sent externally."
    ...
    @classmethod
    def _enrich_growth_blueprint_context(cls, context: dict[str, Any]) -> bool:
        dispatch = context.get("dispatch") if isinstance(context.get("dispatch"), dict) else {}
        evidence = context.get("evidence") if isinstance(context.get("evidence"), dict) else {}
        ...
        if review.status == "ready_for_approval":
            ...
            disposition = "growth_blueprint_revision_required"
            execution_next_action = "Revise the Growth Blueprint against Tony's failed quality checks and return the complete evidence package. Do not prepare or send outreach."
            evidence["execution_next_action"] = execution_next_action
        context["evidence"] = evidence
        context["commercial_judgement"] = {"disposition": disposition, "recommended_next_action": recommendation, "execution_next_action": execution_next_action, "review_status": review.status, "review_checks": dict(review.checks), "failed_checks": list(review.failed_checks), "judgement_owner": "Tony", "evidence_basis": review.evidence_basis}
        return True
