...
from runtime.pipeline_runner import PipelineRunner, load_pipeline_fixture
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the Growth Blueprint pipeline with an OpenAI-compatible live provider"
    )
    ...
    parser.add_argument("--runtime-root", default=".runtime/live-pipeline")
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--provider-id", default="local-openai-compatible")
    parser.add_argument("--model", default="qwen3.5:latest")
    parser.add_argument("--endpoint-env", default="NARRATIIVE_LIVE_ENDPOINT")
    parser.add_argument("--api-key-env", default="NARRATIIVE_LIVE_API_KEY")
    ...
    provider = EnvironmentTextProviderClient(
